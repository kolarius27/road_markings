...
if __name__ == '__main__':
    #arcpy.env.workspace = "C:/Users/micha/Desktop/Gekon/road_markings/RSM_arcgis_project/RSM_arcgis_project.gdb"
    ...
    #open_ruian.ExecuteSQL("SELECT ∗ FROM layer_name WHERE NAZEV = 'Praha11'")
    ...
    # arcpy.env.outputCoordinateSystem = arcpy.Describe(ruian_shp).spatialReference

    # mestska_cast = "p11"
    # mc_extent = "p11_ext"

    #where_clause = '"NAZEV" = \'Praha 11\''

    #if arcpy.Exists(mestska_cast):
        #arcpy.Delete_management(mestska_cast)
    #if arcpy.Exists(mc_extent):
        #arcpy.Delete_management(mc_extent)

    #arcpy.Select_analysis(ruian_shp, mestska_cast, where_clause)

    #arcpy.MinimumBoundingGeometry_management(mestska_cast, mc_extent,
                                             #"ENVELOPE", "NONE")


    #arcpy.AddField_management(output_fc, 'Left', 'FLOAT')
    #arcpy.AddField_management(output_fc, 'Right', 'FLOAT')
    #arcpy.AddField_management(output_fc, 'Bottom', 'FLOAT')
    #arcpy.AddField_management(output_fc, 'Top', 'FLOAT')

    #arcpy.CalculateGeometryAttributes_management(output_fc, [['Left', 'LENGTH']], 'METERS')

    #print(arcpy.Exists(output_fc))

    #minX = [row[0] for row in arcpy.da.SearchCursor(output_fc, 'Left')]
    #maxX = [row[0] for row in arcpy.da.SearchCursor(output_fc, 'Right')]
    #minY = [row[0] for row in arcpy.da.SearchCursor(output_fc, 'Bottom')]
    #maxY = [row[0] for row in arcpy.da.SearchCursor(output_fc, 'Top')]

    #output_desc = arcpy.Describe(output_fc)
    #minX = output_desc.extent.XMin
    #maxX = output_desc.extent.XMax
    #minY = output_desc.extent.YMin
    #maxY = output_desc.extent.YMax
